<template>
  <div class="h-full relative" :data-prevent-clickaway="visible || null">
    <button class="h-full aspect-square -mr-offset relative z-10 flex" @click="visible = !visible">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="m-auto w-6 h-6">
        <transition
          ...
          enter-active-class="origin-center transition-transform duration-200 easing-linear"
          leave-active-class="origin-center transition-transform duration-200 easing-linear"
        >
    ...
    <SlidingTransition :duration="{enter: 300, leave: 200}">
      <div v-if="visible" class="fixed xl:absolute top-0 right-0 w-full md:w-[400px] xl:-mr-offset flex flex-col justify-end">
        <div class="h-screen md:h-auto max-h-screen bg-gray-700 md:rounded-bl-md xl:rounded-br-md md:shadow-lg text-white flex flex-col">
          ...
          <div class="flex-1 md:rounded-bl-md xl:rounded-br-md overflow-auto flex flex-col">
            <section class="py-4 border-b border-transparent">
              <h3 class="mb-4 px-4 text-sm text-gray-400 uppercase tracking-wide">{{ t('nyaa.player_advancements.section_title') }}</h3>
              <label class="px-4 h-12 bg-gray-600 cursor-pointer flex items-center">
                <span>{{ t('nyaa.config.show_all_advancements') }}</span>
                <FormSwitch v-model="config.showAllAdvancements" class="ml-auto" />
              </label>
            </section>
            <section class="py-4 border-b border-transparent">
              <h3 class="mb-4 px-4 text-sm text-gray-400 uppercase tracking-wide">{{ t('nyaa.player_statistics.section_title') }}</h3>
              <label class="px-4 h-12 bg-gray-600 cursor-pointer flex items-center">
            ...
            <section class="py-4 border-b border-transparent">
              <h3 class="mb-4 px-4 text-sm text-gray-400 uppercase tracking-wide">{{ t('nyaa.app_menu.section_title.others') }}</h3>
              <div class="px-4 h-12 bg-gray-600 flex items-center">
            ...
            <footer class="mt-auto flex-none pt-1 pb-5">
              <p class="text-xs text-gray-400 text-center">Made by SilentDepth</p>
              <p class="md:hidden mt-3 text-sm text-gray-400 text-center">&copy; {{ $store.state.info.servername }}</p>
            </footer>
          </div>
        </div>
      </div>
...
<style scoped>
  @utility -mr-offset {
    margin-right: calc(-1 * (var(--spacing-header) - var(--spacing-6)) / 2);
  }
</style>
